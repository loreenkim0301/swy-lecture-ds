...
> **주의**: AI는 코드를 "그대로 복사"하지 않고 요약·재작성하는 경우가 있어서, 위 CDN 주소 두 줄을 옮겨 적는 과정에서 철자나 경로 일부가 바뀔 수 있습니다(예: `swy-lecture-ds`가 `swy-lecturenote`로 바뀌거나, 파일명 뒤 `.min.css`가 `.css`로 바뀌는 식). AI가 준 결과물을 저장하기 전에, `<head>` 안의 두 줄이 위 프롬프트에 적힌 주소와 **글자 하나까지 정확히 같은지** 한 번 대조해보세요. 다르면 페이지가 열려도 디자인이 하나도 적용 안 된 채로 나옵니다 — 그럴 땐 위 두 줄을 통째로 지우고 이 README에 있는 원본으로 다시 붙여넣으면 됩니다.
